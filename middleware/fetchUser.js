...
const JWT_SECRET = process.env.JWT_SECRET; // making .env variable
...
const fetchUser = (req, res, next) => {
  ...
  if (!token) {
    return res.status(401).send({ error: "Please authenticate using a valid token" }); // if token is not available then send error.
  }

  try {console.log(JWT_SECREsT);
